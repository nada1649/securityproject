...
if __name__ == "__main__":
    ...
    folder_path = input("Enter path to folder with encrypted files: ").strip()
    key_hex = input("Enter the encryption key (hex): ").strip()
    nonce_hex = input("Enter the nonce (hex): ").strip()
    
    try:
        key = bytes.fromhex(key_hex)
        nonce = bytes.fromhex(nonce_hex)
    except ValueError:
        print("Invalid key or nonce format. Must be valid hexadecimal.")
        exit(1)
    
    if decrypt_folder_in_place(folder_path, key, nonce):
        print("\nDecryption successful! Files restored to original names.")
    else:
        print("\nDecryption failed. Some files may remain encrypted.")